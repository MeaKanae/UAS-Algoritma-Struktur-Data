...
void setDate(Date *date)
{
    ...
    { // Cek dulu time nya mundur atau ga
...
{
    if (node == NULL)
        return newNode(userType, email, password, name, NIM);
    if (strcmp(email, node->email) < 0)
        node->left = insert(node->left, userType, email, password, name, NIM);

    else if (strcmp(email, node->email) > 0)
        node->right = insert(node->right, userType, email, password, name, NIM);
...
Account *validate_account_input(Account *account, char *email_input, char *password_input)
{
    ...
    while (node != NULL)
    {
        ...
        {
            if (strcmp(password_input, node->password) != 0)
                break;
            Account *user = NULL;
...
int binarySearch(char *targetBook, Book *bookList, int size, char *mode)
{
    ...
    while (low <= high)
    {
        ...
        if (strcmp(mode, "ASCENDING") == 0)
        {
            if (strcmp(bookList[mid].title, targetBook) == 0)
                return mid;
            else if (strcmp(targetBook, bookList[mid].title) < 0)
                high = mid - 1;
            else
                low = mid + 1;
        }
...
int interpolationSearch(int targetBook, Book *bookList, int size)
{
    ...
    do
    {
        ...
        if (bookList[pos].ref_number == targetBook)
            return pos;
        if (bookList[pos].ref_number > targetBook)
            high = pos - 1;
        else
...
void removeBookMenu(Shelf *shelf)
{
    ...
    for (int i = 0; i < shelf->heapSize; i++)
    { // Iterasi dari index 0 sampe ketemu
...
int checkDuplicate(char *target, BorrowBook *bookList, int size)
{
    for (int i = 0; i < size; i++)
    {
        if (strcmp(target, bookList[i].title) == 0)
            return 1;
    }
    return 0;
}
